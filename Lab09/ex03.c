...
//protos
int get_input(LINE *lin);
void tolower_string(char *string);

int main(){
    LINE lines[MAX_LINE+1]; //array to store lines
    printf("Trenitalia by alexcarchiar\n");
    int number_of_lines = get_input(lines); //function to get input from files
    
    /*for(int i = 0; i<number_of_lines; ++i){ //printing all the lines from the file
        printf("%s %s %s %s\n", lines[i].departure_station, lines[i].departure_time, lines[i].arrival_station, lines[i].arrival_time);
    }*/
...
int get_input(LINE *lin){
    ...
    FILE *input = fopen("/Users/alexcarchiar/Desktop/trains.alexcarchiar", "r");
